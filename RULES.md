...
| D6 attribution | `derive()` / `buildLodgment()` | The pool deduction splits in two and only one half can be attributed. The **first-year 18.75%** is each asset's own — cost × work use × 18.75% — so it reports by category. The **ongoing 37.5%** is charged on a balance every asset has been folded into, so it reports as one line, *pool balance brought forward*. Splitting that by category would be an invention. The two always sum to `pool[fy].deduction`. |
