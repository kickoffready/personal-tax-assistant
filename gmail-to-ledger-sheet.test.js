#!/usr/bin/env node
/**
 * Tests for gmail-to-ledger-sheet.gs
 * run: node gmail-to-ledger-sheet.test.js
 *
...
const gsSource = fs.readFileSync(path.join(__dirname, "gmail-to-ledger-sheet.gs"), "utf8");
...
const G = { console, Logger: { log(){} }, GmailApp: {}, DriveApp: {}, Utilities: {}, SpreadsheetApp: {} };
vm.createContext(G);
...
section("— the Gmail query and financial-year boundaries");
const bounds = G.financialYearBounds("2025-26");
t("the FY begins on 1 July and ends before the next 1 July",
  bounds.start === "2025-07-01" && bounds.end === "2026-07-01",
  bounds.start + "/" + bounds.end, "2025-07-01/2026-07-01");
t("the Gmail query includes 1 July through 30 June",
  G.gmailSearchQuery("invoice", "2025-26") ===
    'label:"invoice" after:2025/06/30 before:2026/07/01',
  G.gmailSearchQuery("invoice", "2025-26"), "bounded invoice query");
t("30 June before the FY is excluded",
  !G.messageFallsInFY({ date:new Date(2025,5,30) }, "2025-26"), "excluded", "excluded");
t("1 July opening the FY is included",
  G.messageFallsInFY({ date:new Date(2025,6,1) }, "2025-26"), "included", "included");
t("30 June closing the FY is included",
  G.messageFallsInFY({ date:new Date(2026,5,30) }, "2025-26"), "included", "included");
t("the following 1 July is excluded",
  !G.messageFallsInFY({ date:new Date(2026,6,1) }, "2025-26"), "excluded", "excluded");
let badFY = "";
try { G.financialYearBounds("2025-27"); } catch (e) { badFY = e.message; }
t("an inconsistent FY is rejected", /does not follow/.test(badFY), badFY, "a validation error");

section("— large Gmail searches are read in bounded pages");
{
  const starts = [];
  ...
  G.GmailApp.search = (query, start, max) => {
    starts.push(start + "/" + max);
    return threads.slice(start, start + max);
  };
  const read = G.readFinancialYearMessages({ label:"invoice", financialYear:"2025-26" });
  t("every matching thread is read", read.messages.length === 101, read.messages.length, 101);
  t("the bounded query is passed to Gmail",
    read.query === 'label:"invoice" after:2025/06/30 before:2026/07/01', read.query, "bounded query");
  t("search uses documented bounded ranges", starts.join(",") === "0/100,100/100",
    starts.join(","), "0/100,100/100");
}

section("— conversations are filtered back to individual FY messages");
{
  const message = date => ({
    getId:() => String(date.getTime()), getFrom:() => "Adobe <x@adobe.com>",
    getSubject:() => "receipt", getPlainBody:() => "Total $10.00",
    getBody:() => "", getDate:() => date
  });
  const mixedThread = { getMessages:() => [
    message(new Date(2025,5,30)), message(new Date(2025,6,1)),
    message(new Date(2026,5,30)), message(new Date(2026,6,1))
  ] };
  let page = 0;
  G.GmailApp.getUserLabelByName = () => ({});
  G.GmailApp.search = () => page++ ? [] : [mixedThread];
  const read = G.readFinancialYearMessages({ label:"invoice", financialYear:"2025-26" });
  t("only the two messages inside the FY survive the matching thread",
    read.messages.length === 2 && read.messages.every(m => G.fyOf(G.isoDate(m.date)) === "2025-26"),
    read.messages.map(m => G.isoDate(m.date)).join(","), "2025-07-01,2026-06-30");
}
...
section("— the review sheet is a parsed view of the same conversion");
t("the review includes written and skipped messages",
  built.review.length === 5 && built.review.filter(r => r.status === "Skipped").length === 1,
  built.review.length + "/" + built.review.map(r => r.status).join(","), "5/one skipped");
const reviewValues = G.reviewSheetValues(built.review);
t("the review has one stable CSV-friendly header",
  reviewValues[0].join(",") ===
    "Status,Collection,Date,Supplier or item,Issuer,Amount AUD,Work use %,Return label,Review reason,Gmail link,Message ID",
  reviewValues[0].join(","), "the review contract");
t("review rows are sorted by date",
  reviewValues.slice(1).every((row, i, rows) => !i || rows[i - 1][2] <= row[2]),
  reviewValues.slice(1).map(r => r[2]).join(","), "ascending dates");
const privateBuilt = G.buildImportFile([
  msg({ from:"Adobe <x@adobe.com>", body:"Total $12.00 PRIVATE EMAIL BODY" })
], "2025-26");
t("the review never retains the email body",
  !JSON.stringify(privateBuilt.review).includes("PRIVATE EMAIL BODY"),
  JSON.stringify(privateBuilt.review), "no message body");
t("formula-like email text is forced to a literal cell",
  ["=IMPORTXML(1)", "+SUM(1)", "-1+2", "@payload"].every(x => G.sheetLiteral(x) === "'" + x) &&
    G.sheetLiteral("Adobe") === "Adobe",
  G.sheetLiteral("=IMPORTXML(1)"), "formula prefixes quoted");

section("— only the dedicated review tab is refreshed");
{
  const calls = [];
  let written = null;
  const generatedTab = {
    clearContents(){ calls.push("clearContents"); },
    getRange(row, col, rows, cols){
      calls.push("getRange:" + [row,col,rows,cols].join("/"));
      return { setValues(values){ written = values; calls.push("setValues"); } };
    },
    setFrozenRows(n){ calls.push("setFrozenRows:" + n); },
    autoResizeColumns(start, count){ calls.push("autoResizeColumns:" + start + "/" + count); }
  };
  const spreadsheet = {
    getSheetByName(name){ calls.push("getSheetByName:" + name); return null; },
    insertSheet(name){ calls.push("insertSheet:" + name); return generatedTab; },
    getActiveSheet(){ calls.push("UNSAFE getActiveSheet"); throw new Error("must not use active sheet"); }
  };
  G.SpreadsheetApp.getActiveSpreadsheet = () => spreadsheet;
  G.writeReviewSheet(built.review, "Gmail invoice review");
  t("a missing generated tab is created without touching the active tab",
    calls.includes("insertSheet:Gmail invoice review") && !calls.includes("UNSAFE getActiveSheet"),
    calls.join(","), "dedicated tab only");
  t("the review is written in one batch",
    calls.filter(x => x === "setValues").length === 1 && written.length === built.review.length + 1,
    calls.join(",") + "/" + (written && written.length), "one batch/all rows");

  G.SpreadsheetApp.getActiveSpreadsheet = () => null;
  let unbound = "";
  try { G.writeReviewSheet([], "Gmail invoice review"); } catch (e) { unbound = e.message; }
  t("a standalone project gets a useful bound-Sheet error",
    /Extensions/.test(unbound) && /active spreadsheet/.test(unbound), unbound, "bound-Sheet guidance");
}
...
section("— one run produces aligned JSON and review output");
{
  const original = {
    resolvedConfig:G.resolvedConfig,
    readFinancialYearMessages:G.readFinancialYearMessages,
    writeReviewSheet:G.writeReviewSheet,
    writeToDrive:G.writeToDrive,
    main:G.main
  };
  let sheetReview = null, driveJson = null, driveWrites = 0;
  G.resolvedConfig = () => ({
    label:"invoice", financialYear:"2025-26", driveFolder:"", reviewSheetName:"Gmail invoice review"
  });
  G.readFinancialYearMessages = () => ({
    query:'label:"invoice" after:2025/06/30 before:2026/07/01',
    messages:[msg({ id:"aligned-1", from:"Adobe <x@adobe.com>", body:"Total $89.00" })]
  });
  G.writeReviewSheet = (review, name) => { sheetReview = { review, name }; };
  G.writeToDrive = (name, content) => {
    driveWrites++;
    driveJson = JSON.parse(content);
    return { getUrl:() => "https://drive.test/" + name };
  };
  original.main();
  t("main writes one JSON file and one review tab from the same source row",
    driveWrites === 1 && sheetReview.review.length === 1 &&
      driveJson.expenses[0].source_ref === sheetReview.review[0].sourceRef,
    driveWrites + "/" + JSON.stringify(sheetReview) + "/" + JSON.stringify(driveJson),
    "one aligned row in both outputs");

  G.readFinancialYearMessages = () => ({
    query:'label:"invoice" after:2025/06/30 before:2026/07/01',
    messages:[msg({ id:"skipped-1", from:"Adobe <x@adobe.com>", body:"Open online" })]
  });
  original.main();
  t("an all-skipped run refreshes review without creating empty JSON",
    driveWrites === 1 && sheetReview.review.length === 1 && sheetReview.review[0].status === "Skipped",
    driveWrites + "/" + JSON.stringify(sheetReview), "review only");

  G.readFinancialYearMessages = () => ({
    query:'label:"invoice" after:2025/06/30 before:2026/07/01', messages:[]
  });
  original.main();
  t("an empty search refreshes a header-only review without JSON",
    driveWrites === 1 && sheetReview.review.length === 0,
    driveWrites + "/" + JSON.stringify(sheetReview), "empty review only");

  let aliasCalls = 0;
  G.main = () => { aliasCalls++; return "alias-result"; };
  t("extractFYInvoices is a compatibility alias for main",
    G.extractFYInvoices() === "alias-result" && aliasCalls === 1,
    aliasCalls, 1);

  G.resolvedConfig = original.resolvedConfig;
  G.readFinancialYearMessages = original.readFinancialYearMessages;
  G.writeReviewSheet = original.writeReviewSheet;
  G.writeToDrive = original.writeToDrive;
  G.main = original.main;
}
